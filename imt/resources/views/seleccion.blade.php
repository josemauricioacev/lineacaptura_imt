...
<head>
  ...
  <style>
    ...
    body, h1,h2,h3,h4,h5,h6,p,a,li,label,input,select,button,th,td{ font-family:"Montserrat",system-ui,-apple-system,"Segoe UI",Roboto,Arial,sans-serif!important; }
    ...
    @media (max-width:768px){ .step{ min-width:180px; } .form-inline-imt{ flex-direction:column; align-items:stretch; } .btn{ width:100%; } }
...
<main class="page" role="main" style="margin-top:30px">
  <div class="container">
    ...
    <form id="form-next" action="{{ route('seleccion.next') }}" method="POST" novalidate>
      ...
      @error('concepto_id') <small class="form-help" style="display:block">{{ $message }}</small> @enderror

      <div id="preview" class="precio-preview" style="display:none">
...
<script>
  ...
  sel.addEventListener('change', function(){
    if(!sel.value){ setError(true, 'Seleccione un concepto para continuar.'); preview.style.display='none'; return; }
    setError(false, '');
